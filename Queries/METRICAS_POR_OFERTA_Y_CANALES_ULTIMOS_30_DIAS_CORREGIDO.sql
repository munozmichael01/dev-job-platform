-- ================================================================
-- QUERY FINAL CORREGIDA: MÉTRICAS POR CANAL CON JOB TITLES CORRECTOS
-- Usa JobTitles + JobTitlesDenominations en lugar de TJobvacancy.Title
-- ================================================================
...
-- ========== NUEVO CTE: JOB TITLES DEDUPLICADOS ==========
UniqueJobTitles AS (
  SELECT
    jtd.FK_JobTitle,
    jtd.Denomination,
    ROW_NUMBER() OVER (
      ...
      ORDER BY
        ...
        CASE WHEN jtd.BaseName = 1 THEN 0 ELSE 1 END,     -- Priorizar BaseName
        jtd.ID
    ) AS rn
...
-- ========== QUERY FINAL: MÉTRICAS + DIMENSIONES + JOB TITLES CORRECTOS ==========
SELECT
  -- MÉTRICAS POR CANAL
  ...
  -- DIMENSIONES DE LA OFERTA (JOB TITLE CORREGIDO)
  ISNULL(ujt.Denomination, jo.Title) AS JobTitle,  -- ✅ Usa JobTitlesDenominations, fallback a Title
  jo.IDJobTitle AS JobTitleId,  -- ✅ Añadido para referencia
...
-- ✅ JOIN CON JOB TITLES CORRECTOS (NUEVO)
LEFT JOIN UniqueJobTitles ujt ON jo.IDJobTitle = ujt.FK_JobTitle AND ujt.rn = 1

-- Resto de dimensiones (usando CTEs deduplicados)
...
-- CAMBIOS PRINCIPALES:
-- ================================================================
-- 1. ✅ Añadido CTE UniqueJobTitles que une JobTitles + JobTitlesDenominations
-- 2. ✅ Prioriza denominaciones en español (LanguageId = 14)
-- 3. ✅ Prioriza BaseName = 1 (denominación principal)
-- 4. ✅ Añadido campo JobTitleId para referencia
-- 5. ✅ JobTitle ahora usa ujt.Denomination con fallback a jo.Title
-- 6. ✅ Solo usa JobTitles activos (Active = 1)
-- ================================================================
